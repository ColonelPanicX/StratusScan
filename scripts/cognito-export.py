...
def collect_user_pools(regions: List[str]) -> List[Dict[str, Any]]:
    """Collect Cognito user pool information from AWS regions."""
    all_pools = []

    for region in regions:
        utils.log_info(f"Collecting user pools in {region}...")
...
def collect_identity_pools(regions: List[str]) -> List[Dict[str, Any]]:
    """Collect Cognito identity pool (federated identities) information."""
    all_identity_pools = []

    for region in regions:
        utils.log_info(f"Collecting identity pools in {region}...")
...
def collect_user_pool_clients(regions: List[str]) -> List[Dict[str, Any]]:
    """Collect Cognito user pool client (app) information."""
    all_clients = []

    for region in regions:
        utils.log_info(f"Collecting user pool clients in {region}...")
        cognito_client = utils.get_boto3_client('cognito-idp', region_name=region)
...
def collect_identity_providers(regions: List[str]) -> List[Dict[str, Any]]:
    """Collect Cognito identity provider information (SAML, OIDC, Social)."""
    all_providers = []

    for region in regions:
        utils.log_info(f"Collecting identity providers in {region}...")
        cognito_client = utils.get_boto3_client('cognito-idp', region_name=region)
...
def collect_user_pool_groups(regions: List[str]) -> List[Dict[str, Any]]:
    """Collect Cognito user pool group information."""
    all_groups = []

    for region in regions:
        utils.log_info(f"Collecting user pool groups in {region}...")
        cognito_client = utils.get_boto3_client('cognito-idp', region_name=region)
