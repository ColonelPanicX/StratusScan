...
@utils.aws_error_handler("Collecting WAF web ACLs", default_return=[])
def collect_web_acls(regions: List[str], scope: str = 'REGIONAL') -> List[Dict[str, Any]]:
    """
    Collect WAF web ACL information from AWS regions.
        ...
        try:
            ...
            for page in paginator.paginate(Scope=scope):
                ...
                for acl_summary in web_acls:
                    ...
                    print(f"  Processing Web ACL: {acl_name}")
            ...
            utils.log_error(f"Error processing region {region} for WAF web ACLs", e)

    utils.log_success(f"Total WAF web ACLs ({scope}) collected: {len(all_web_acls)}")
    return all_web_acls


@utils.aws_error_handler("Collecting WAF rules", default_return=[])
def collect_waf_rules(regions: List[str], scope: str = 'REGIONAL') -> List[Dict[str, Any]]:
    """
    Collect WAF rule information from web ACLs.
    ...
    for region in regions:
        ...
        try:
            ...
            utils.log_error(f"Error collecting WAF rules in region {region}", e)
...
def collect_ip_sets(regions: List[str], scope: str = 'REGIONAL') -> List[Dict[str, Any]]:
    """
    Collect WAF IP set information.
