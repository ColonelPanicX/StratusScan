...
def collect_kms_keys(regions: List[str], account_id: str) -> List[Dict[str, Any]]:
    """
    Collect KMS key information from AWS regions.
            ...
            for page in paginator.paginate():
                ...
                for key in keys:
                    ...
                    try:
                        ...
                        utils.log_warning(f"Could not get metadata for key {key_id}: {e}")

            print(f"  Found {key_count} KMS keys")
...
def collect_kms_aliases(regions: List[str]) -> List[Dict[str, Any]]:
    """
    Collect KMS key alias information from AWS regions.
...
def collect_kms_grants(regions: List[str]) -> List[Dict[str, Any]]:
    """
    Collect KMS grant information from AWS regions.
...
def export_kms_data(account_id: str, account_name: str):
    ...
    print(f"\nStarting KMS export process for {region_text}...")
