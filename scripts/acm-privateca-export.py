...
def collect_private_cas(regions: List[str]) -> List[Dict[str, Any]]:
    ...
    for region in regions:
        utils.log_info(f"Collecting Private CAs in {region}...")
    ...
    utils.log_info(f"Collected {len(all_cas)} Private CAs")
    return all_cas
...
def collect_issued_certificates(regions: List[str]) -> List[Dict[str, Any]]:
    ...
    for region in regions:
        utils.log_info(f"Collecting issued certificates in {region}...")
        acmpca_client = utils.get_boto3_client('acm-pca', region_name=region)
    ...
    utils.log_info(f"Collected {len(all_certificates)} certificates (sample limited to 50 per CA)")
...
def collect_certificate_templates(regions: List[str]) -> List[Dict[str, Any]]:
    ...
    for region in regions:
        utils.log_info(f"Collecting certificate templates in {region}...")
        acmpca_client = utils.get_boto3_client('acm-pca', region_name=region)
    ...
    utils.log_info(f"Collected {len(all_templates)} certificate templates")
...
def collect_ca_permissions(regions: List[str]) -> List[Dict[str, Any]]:
    ...
    for region in regions:
        utils.log_info(f"Collecting CA permissions in {region}...")
        acmpca_client = utils.get_boto3_client('acm-pca', region_name=region)
    ...
    utils.log_info(f"Collected {len(all_permissions)} CA permission statements")
...
def main():
    ...
    if choice == '1':
        regions = utils.get_all_aws_regions(service_code='acm-pca')
