...
@utils.aws_error_handler("Collecting Network Firewalls", default_return=[])
def collect_network_firewalls(regions: List[str]) -> List[Dict[str, Any]]:
    """
    Collect Network Firewall information from AWS regions.
    ...
    for region in regions:
        ...
        try:
            ...
            print(f"  Found {firewall_count} Network Firewalls")

        except Exception as e:
            utils.log_error(f"Error processing region {region} for Network Firewalls", e)

    utils.log_success(f"Total Network Firewalls collected: {len(all_firewalls)}")
    return all_firewalls


@utils.aws_error_handler("Collecting firewall policies", default_return=[])
def collect_firewall_policies(regions: List[str]) -> List[Dict[str, Any]]:
    """
    Collect Network Firewall policy information from AWS regions.
    ...
    for region in regions:
        ...
        try:
            ...
            for page in paginator.paginate():
                policies = page.get('FirewallPolicies', [])
            ...
            print(f"  Found {policy_count} firewall policies")

        except Exception as e:
            utils.log_error(f"Error processing region {region} for firewall policies", e)

    utils.log_success(f"Total firewall policies collected: {len(all_policies)}")
    return all_policies


@utils.aws_error_handler("Collecting rule groups", default_return=[])
def collect_rule_groups(regions: List[str]) -> List[Dict[str, Any]]:
    """
    Collect Network Firewall rule group information from AWS regions.
    ...
    for region in regions:
        ...
        try:
            ...
            for page in paginator.paginate():
                ...
                rg_count += len(rule_groups)

                for rg_metadata in rule_groups:
            ...
            print(f"  Found {rg_count} rule groups")

        except Exception as e:
            utils.log_error(f"Error processing region {region} for rule groups", e)

    utils.log_success(f"Total rule groups collected: {len(all_rule_groups)}")
    return all_rule_groups


@utils.aws_error_handler("Collecting logging configurations", default_return=[])
def collect_logging_configurations(regions: List[str]) -> List[Dict[str, Any]]:
    """
    Collect Network Firewall logging configuration information from AWS regions.
    ...
    for region in regions:
        ...
        try:
            ...
            utils.log_error(f"Error processing region {region} for logging configurations", e)
...
def export_network_firewall_data(account_id: str, account_name: str):
    ...
    print(f"\nStarting Network Firewall export process for {region_text}...")
