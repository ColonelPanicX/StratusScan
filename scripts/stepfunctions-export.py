...
def collect_state_machines(regions: List[str]) -> List[Dict[str, Any]]:
    ...
    for region in regions:
        utils.log_info(f"Scanning Step Functions state machines in {region}...")
        ...
        for page in paginator.paginate():
            ...
            for sm_summary in state_machines:
                state_machine_arn = sm_summary.get('stateMachineArn', 'N/A')
                name = sm_summary.get('name', 'N/A')
                sm_type = sm_summary.get('type', 'N/A')  # STANDARD or EXPRESS
                ...
                try:
                    ...
                    # Definition (state machine workflow)
                    definition = sm_response.get('definition', 'N/A')

                    # Count states in definition
                    state_count = 'N/A'
                    try:
                        if definition != 'N/A':
                            definition_json = json.loads(definition)
                            states = definition_json.get('States', {})
                    ...
                    include_execution_data = logging_config.get('includeExecutionData', False)

                    # CloudWatch log destinations
                    log_destinations = logging_config.get('destinations', [])
                    log_group_arns = [dest.get('cloudWatchLogsLogGroup', {}).get('logGroupArn', '')
                    ...
                    log_groups_str = ', '.join(log_groups) if log_groups else 'None'
                    ...
                    all_state_machines.append({
                        ...
                        'Include Execution Data': 'Yes' if include_execution_data else 'No',
                        'Log Groups': log_groups_str,
                        'X-Ray Tracing': 'Enabled' if tracing_enabled else 'Disabled',
                        'Label': label,
        ...
        utils.log_success(f"Collected {len([sm for sm in all_state_machines if sm['Region'] == region])} Step Functions state machines from {region}")
...
def collect_activities(regions: List[str]) -> List[Dict[str, Any]]:
    ...
    for region in regions:
        utils.log_info(f"Scanning Step Functions activities in {region}...")
        sfn_client = utils.get_boto3_client('stepfunctions', region_name=region)
        ...
        for page in paginator.paginate():
            ...
            for activity in activities:
                activity_arn = activity.get('activityArn', 'N/A')
        ...
        utils.log_success(f"Collected {len([a for a in all_activities if a['Region'] == region])} Step Functions activities from {region}")
...
def main():
    ...
    if region_choice == '1':
        # Get all AWS regions
        regions = utils.get_all_aws_regions('states')  # 'states' is the service code for Step Functions
